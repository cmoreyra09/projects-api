...
def getConfigOnline(logger):
    ...
    with dbEngine.connectDb().connect() as connection:
        query = '''
            ...
            FROM tbl_configuration_gocontact WHERE stateReg = 1 AND flatInterval = 1
        '''
...
async def workFlow_ApiGoContactOnline(serviceName, dbConfig, logConfig, spSyncExecution):
    
    
    await workflowGetApiGoContact(online=True)
...
if __name__ == "__main__":
    serviceName = "workflow_gocontact_online"
    ...
    asyncio.run(workFlow_ApiGoContactOnline(serviceName, dbConfig, logConfig, spSyncExecution=True))
