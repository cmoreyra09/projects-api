...
banner                              =           pyfiglet.figlet_format(f"goAnalytics-Agents")
...
logger                              =           setupLoggerTask(mode="Agents")
...
async def getConfiguration():
    ...
    SELECT DISTINCT 
        ...
        url + '/poll/api/goanalytics/agents' AS epAgents
...
async def getApi(session, endpoint, token, config):
    ...
    try:
        async with session.get(endpoint, headers=headers) as response:
            ...
            # Añadir columnas 'campaing' y 'domaing'
            for record in data:
...
async def workFlow_ApiDimAgentesGA(save_mode="consolidated"):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if save_mode == "consolidated" and allDataTarget:
            ...
            # Guardar como CSV
            output_csv_file = os.path.join(baseFolder, f"DimAgents.txt")
            os.makedirs(baseFolder, exist_ok=True)
            df.to_csv(output_csv_file, index=False, sep="|")
            
            logger.info(f"Datos consolidados guardados en un archivo CSV: '{output_csv_file}'")
            print(f"Datos consolidados guardados en un archivo CSV: '{output_csv_file}'")
            
            # Guardar como JSON
            output_json_file = os.path.join(baseFolderOriginal, f"DimAgents.json")
            ...
            FROM tbl_configuration_workloads_bi WHERE id = 9
            '''
...
if __name__ == "__main__":
    asyncio.run(workFlow_ApiDimAgentesGA('consolidated'))
